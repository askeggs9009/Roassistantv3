...
// FIXED: Enhanced Google OAuth callback with comprehensive logging
app.get("/auth/google/callback", async (req, res) => {
    // Ensure FRONTEND_URL is properly set
    const frontendUrl = process.env.FRONTEND_URL || 'https://musical-youtiao-b05928.netlify.app';
    
    console.log('[OAUTH CALLBACK] Starting processing...');
    console.log('[OAUTH CALLBACK] Frontend URL:', frontendUrl);
    console.log('[OAUTH CALLBACK] Query params:', Object.keys(req.query));
    ...
    try {
        ...
        console.log('[OAUTH] User info retrieved:', {
            email: data.email,
            name: data.name,
            hasProfilePicture: !!data.picture
        });
        ...
        // Prepare minimal user data for frontend (to avoid URL length issues)
        ...
        // Encode user data - keep it minimal to avoid URL length issues
        const userDataEncoded = encodeURIComponent(JSON.stringify(userDataForFrontend));
        
        console.log('[OAUTH] User data encoded, length:', userDataEncoded.length);
        
        // Build redirect URL
        const redirectUrl = `${frontendUrl}/login-success.html?token=${token}&user=${userDataEncoded}`;
        
        console.log('[OAUTH] Full redirect URL length:', redirectUrl.length);
        console.log('[OAUTH] Redirect URL (first 200 chars):', redirectUrl.substring(0, 200));
        
        // Check if URL might be too long (browsers typically support up to 2048 characters)
        if (redirectUrl.length > 2000) {
            console.warn('[WARNING] Redirect URL is very long, might cause issues');
            
            // Alternative: Store data temporarily and use a session key
            ...
            // Store temporarily
            global.tempOAuthSessions = global.tempOAuthSessions || new Map();
            global.tempOAuthSessions.set(sessionKey, tempSession);
            
            // Clean up old sessions
            for (const [key, session] of global.tempOAuthSessions.entries()) {
                if (Date.now() > session.expires) {
                    global.tempOAuthSessions.delete(key);
                }
            }
            
            const shortRedirectUrl = `${frontendUrl}/login-success.html?session=${sessionKey}`;
            console.log('[OAUTH] Using session-based redirect:', shortRedirectUrl);
            
            return res.redirect(shortRedirectUrl);
        }
        
        // Perform the redirect
        console.log('[OAUTH] Sending redirect response...');
        res.redirect(redirectUrl);
        console.log('[OAUTH] Redirect sent successfully');
        ...
        // Always redirect somewhere on error
        const errorMessage = encodeURIComponent(error.message || 'Unknown error');
        const errorRedirect = `${frontendUrl}/login.html?error=google_auth_failed&details=${errorMessage}`;
        console.log('[ERROR] Redirecting to login with error:', errorRedirect);
        res.redirect(errorRedirect);
    }
});

// Add this new endpoint to handle session-based OAuth completion
app.get("/auth/oauth-session/:sessionKey", (req, res) => {
    const { sessionKey } = req.params;
    
    if (!global.tempOAuthSessions || !global.tempOAuthSessions.has(sessionKey)) {
        return res.status(404).json({ error: 'Session not found or expired' });
    }
    
    const session = global.tempOAuthSessions.get(sessionKey);
    global.tempOAuthSessions.delete(sessionKey); // Use once only
...
// Enhanced startup function with better environment checking
async function startServer() {
    ...
    console.log(`FRONTEND_URL: ${process.env.FRONTEND_URL || 'NOT SET (using default: https://musical-youtiao-b05928.netlify.app)'}`);
    console.log(`BASE_URL: ${process.env.BASE_URL || 'NOT SET (auto-detected)'}`);
    ...
    if (!process.env.FRONTEND_URL) {
        console.warn('\n[WARNING] FRONTEND_URL not set in environment variables!');
        console.warn('[WARNING] Using default: https://musical-youtiao-b05928.netlify.app');
        console.warn('[WARNING] Set FRONTEND_URL in Railway environment variables for production');
    }
    ...
    app.listen(port, '0.0.0.0', () => {
        ...
        console.log(`   Frontend URL: ${process.env.FRONTEND_URL || 'https://musical-youtiao-b05928.netlify.app'}`);
